...
const LoginComponent = ()=>{
    ...
    //애플리케이션 상태변경(로그인), 로그인후 이동 -> 커스텀 훅 이용
    ...
    const handleClickLogin = (e) =>{
       // dispatch(login(loginParam)) //loginSlice.login()--동기화된 호출
       // 비동기 호출로 로그인 unwrap(): 호출처리 결과 사용 -> 커스텀 훅 사용
       doLogin(loginParam)
       .then(data=>{
        ...
        if(data.error){
            ...
            alert("로그인 성공")
            moveToPath('/')
            //navigate({pathname:`/`}, {replace:true}) //뒤로가기 시 로그인화면 볼 수 없게
        }
       })
    }
